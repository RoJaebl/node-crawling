...
import { crawlingStore, ICategory, SET } from "./store.js";
import { CATEGORY_PATH, hover } from "./crawling.js";
import { Store, AnyAction, Action } from "redux";

// category xpath
const propsXPath = {
    categoryButton: `_categoryButton_category_button_1lIOy`,
...
export const getCategories = async (driver: WebDriver) => {
    ...
    await driver.findElement(By.className(propsXPath.categoryButton)).click();
    // wait for category layer
    await driver.sleep(100);
    // get major categories
    const majors = await driver.findElements(By.xpath(propsXPath.major));
    ...
    for await (const major of majors) {
        ...
        const minors = await driver.findElements(By.xpath(propsXPath.minor));
        if (
        ...
        for await (const minor of minors) {
            ...
            await driver.sleep(100);
            const subs = await driver.findElements(By.xpath(propsXPath.sub));
